...
class WinFromIframeTest {
    ...
    void shouldProcessWinFromIframeAndVerifyEvent() {
        ...
        testData.expectedBalance = adjustmentAmount.subtract(betAmount).add(winAmount);
        ...
        step("NATS: Проверка поступления события won_from_iframe", () -> {
            ...
            var actualPayload = testData.winEvent.getPayload();
        ...
        step("Kafka: Проверка поступления сообщения won_from_iframe в топик wallet.v8.projectionSource", () -> {
            ...
            assertTrue(utils.areEquivalent(kafkaMessage, testData.winEvent), "kafka.payload");
        });
        ...
        step("DB Wallet: Проверка записи в таблице betting_projection_iframe_history", () -> {
            var dbTransaction = walletDatabaseClient.findLatestIframeHistoryByUuidOrFail(
                    testData.winEvent.getPayload().getUuid());

            var winEventPayload = testData.winEvent.getPayload();
            var player = testData.registeredPlayer.getWalletData();
            var betInfo = winEventPayload.getBetInfo().getFirst();
            ...
            assertAll("Проверка записанной строки в таблицу с историей ставок на спорт",
                    ...
                    () -> assertEquals(testData.winEvent.getSequence(), dbTransaction.getSeq(), "db.iframe_history.seq"),
        ...
        step("Redis(Wallet): Получение и проверка полных данных кошелька", () -> {
            ...
            var expectedBetInfo = testData.winEvent.getPayload();
