...
class CasinoLossWhenLossFromIframeTest extends BaseTest {
    ...
    void shouldRejectBetWhenGamblingDisabled() {
        ...
        testData.expectedSpent = betAmount;
        ...
        step("Manager API: Получение проигрыша", () -> {
            ...
            step("NATS: Проверка поступления события loosed_from_iframe", () -> {
                ...
                testData.lossEvent = natsClient.findMessageAsync(
                        subject,
                        NatsBettingEventPayload.class,
                        filter).get();

                assertNotNull(testData.lossEvent, "nats.event.loosed_from_iframe");
            });
        });
