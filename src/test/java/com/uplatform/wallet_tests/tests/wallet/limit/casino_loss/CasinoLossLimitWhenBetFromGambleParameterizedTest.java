...
@TmsLink("NW-38")
class CasinoLossLimitWhenBetFromGambleParameterizedTest {
    ...
    void test(
        ...
        testData.expectedSpentAmountAfterBet = testData.betAmount;
        testData.expectedRestAmountAfterBet = testData.limitAmount.subtract(testData.betAmount);
        ...
        step("Manager API: Совершение ставки", () -> {
            ...
            step("Sub-step NATS: Проверка поступления события betted_from_gamble", () -> {
                ...
                assertNotNull(testData.betEvent, "nats.event.betted_from_gamble");
            });
        });
        ...
        step("Redis(Wallet): Проверка изменений лимита в агрегате", () -> {
            ...
            assertAll(
                    () -> assertEquals(0, testData.expectedRestAmountAfterBet.compareTo(aggregate.getLimits().getFirst().getRest()), "redis.wallet.limit.rest"),
                    () -> assertEquals(0, testData.expectedSpentAmountAfterBet.compareTo(aggregate.getLimits().getFirst().getSpent()), "redis.wallet.limit.spent"),
