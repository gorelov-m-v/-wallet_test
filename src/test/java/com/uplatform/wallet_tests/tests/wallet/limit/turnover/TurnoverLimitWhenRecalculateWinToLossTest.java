...
class TurnoverLimitWhenRecalculateWinToLossTest extends BaseTest {
    ...
    void shouldMaintainTurnoverLimitStateAfterBetWinAndRecalculationToLoss() {
        ...
        step("Manager API: Перерасчет результата на проигрыш (LOSS)", () -> {
            ...
            step("Sub-step NATS: Проверка поступления события recalculated_from_iframe", () -> {
                ...
                assertAll("nats.recalculated_from_iframe_event.content_validation",
                        ...
                        () -> assertEquals(0, winAmountRecalculated.compareTo(testData.recalculatedEvent.getPayload().getAmount().negate()), "nats.recalculated_from_iframe_event.payload.amount"),
                        () -> assertEquals(NatsBettingTransactionOperation.LOSS, testData.recalculatedEvent.getPayload().getType(), "nats.recalculated_from_iframe_event.payload.operation"),
