    ...
    void shouldNotChangeTurnoverLimitOnLoss() {
        ...
        step("Manager API: Получение проигрыша по ставке (LOSS)", () -> {
            ...
            step("Sub-step NATS: Проверка поступления события loosed_from_iframe", () -> {
                ...
                testData.lossEvent = natsClient.findMessageAsync(subject, NatsBettingEventPayload.class, filter).get();

                assertAll("nats.loosed_from_iframe_event.content_validation",
                        () -> assertNotNull(testData.lossEvent, "nats.loosed_from_iframe_event"),
                        () -> assertEquals(0, lossAmount.compareTo(testData.lossEvent.getPayload().getAmount()), "nats.loosed_from_iframe_event.payload.amount"),
                        () -> assertEquals(NatsBettingTransactionOperation.LOSS, testData.lossEvent.getPayload().getType(), "nats.loosed_from_iframe_event.payload.operation"),
                        () -> assertEquals(testData.betRequestBody.getBetId(), testData.lossEvent.getPayload().getBetId(), "nats.loosed_from_iframe_event.payload.betId")
                );
            });
        });
        ...
        step("Redis(Wallet): Проверка изменений лимита в агрегате после всех операций", () -> {
            ...
            assertAll("redis.wallet.limit_data_validation",
                    () -> assertEquals((int) testData.lossEvent.getSequence(), aggregate.getLastSeqNumber(), "redis.wallet.last_seq_number"),
